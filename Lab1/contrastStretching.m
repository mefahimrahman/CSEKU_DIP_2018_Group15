image=imread('logTransformation.jpg');% To read image
figure,imshow(image)
grayImage=rgb2gray(image);% To convert RGB image to gray image(normalised image)
...
[rowi,coli]=size(grayImage);
out=zeros(rowi,coli);

for k=1:256
    for i=1:rowi
        for j=1:coli
            if grayImage(i,j)==x(k) 
                out(i,j)=y(k);
            end
        end
    end
